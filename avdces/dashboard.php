...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
	...
	?>
		...
		for ($i=0; $i<mysql_num_rows($result_net); $i++){
			...
			$sql_cpu = "SELECT vm_cpu FROM vm where vm_net = '$net_name'";
			
			$result_cpu = mysql_query($sql_cpu) or die ("can't select cpu");
			
			for ($ii = 0; $ii < mysql_num_rows($result_cpu); $ii++){
	
					$row_cpu = mysql_fetch_row($result_cpu);
					
					list($vm_cpu) = $row_cpu;
					
					#echo $vm_cpu;
					
					$net_cpu = $net_cpu + $vm_cpu;
