...
if(!empty($_POST['del_vm_list'])) {
		
		foreach($_POST['del_vm_list'] as $vm_name) {
			
			echo "delete vm ".$vm_name."<p>";
			#vm_del($vm_name);
			...
			#force shut down vm
			$ret = $lv->domain_destroy($vm_name) ? "Domain has been destroyed successfully" : 
			...
			#delete image
			exec("ssh -o StrictHostKeyChecking=no root@127.0.0.1 'rm -fr /var/lib/libvirt/images/$vm_name.qcow2' ");

			# delete from db
			$sql_del_vm = "delete from vm where vm_name = '$vm_name'";
			mysql_query($sql_del_vm)or die("sql can't delete vm $vm_name");
			echo $vm_name." is deleted!<p>";

		}
}else{
	
	print_r($_POST['del_vm_list']);
